...
class TweetSetSuite extends FunSuite {
  ...
  test("filter: a on set5") {
    new TestSets {
      set5.filter(tw => tw.user == "a").foreach( tw => print (tw.toString) )
  ...
  test("union: with set (2) and set (3)") {
    new TestSets {
      assert(size(set2.union(set3)) === 2)
    }
  }

  test("union: with set (2) and set (2)") {
    new TestSets {
  ...
  test("mostRetweeted: set1 failure") {
    new TestSets {
  ...
  test("mostRetweeted: set7") {
    new TestSets {
      assert(set7.mostRetweeted.retweets == 9)
    }
  }
